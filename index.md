...
copula framework. Centered on a single core function,
[`makeData()`](https://boehringer-ingelheim.github.io/endpoints/reference/makeData.md),
it provides a unified interface for generating continuous, binary,
count, and time-to-event outcomes, while also supporting multiple
treatment arms, censoring, stochastic enrollment, subject-level
follow-up limits, and trial-ending rules. The package is designed to
produce realistic patient-level datasets with configurable treatment
effects, dependence structures, and trial-calendar features for trial
...
- Time-to-event functionality including independent censoring,
  fatal/non-fatal endpoint logic for semi-competing risks settings
- Trial-calendar functionality through `enrollment_details`,
  `followup_details`, and `trial_end_details`
- Stochastic enrollment with exponential and piecewise Poisson-process
  accrual mechanisms
- Subject-level follow-up limits and trial-ending rules, including fixed
  calendar end, last-patient-minimum-follow-up, and event-driven designs
...
- [Stochastic Enrollment, Follow-up and Trial
  Calendar](https://boehringer-ingelheim.github.io/endpoints/articles/enrollment_guide.html)
